...
async function fetchGameCoverFromTheGamesDB(title: string, systemName: string, apiKey?: string): Promise<string> {
  ...
  try {
    ...
    const platformId = platformIds.length > 0 ? platformIds[0] : undefined

    // Use our proxy API route instead of calling TheGamesDB directly
    const searchResponse = await fetch("/api/thegamesdb/search", {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        title,
        systemName: platformId?.toString(),
        apiKey,
      }),
    })
